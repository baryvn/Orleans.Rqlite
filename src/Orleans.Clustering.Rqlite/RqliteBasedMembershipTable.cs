...
namespace Orleans.Runtime.Membership
{
    ...
    public class RqliteBasedMembershipTable : IMembershipTable
    {
        ...
        public async Task CleanupDefunctSiloEntries(DateTimeOffset beforeDate)
        {
            using (var _rqlite = new RqliteNetClient(this.Uri))
            {
                await _rqlite.Execute(string.Format("DELETE FROM  " + ClusterId + "_Members WHERE json_extract(Data,'$.Status') = {0} AND json_extract(Data,'$.IAmAliveTime') < '{1}'", (int)SiloStatus.Dead, beforeDate.ToString("o")));
            }
        }
    }
